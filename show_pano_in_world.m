...
%SHOW_PANO_IN_WORLD - show panoramatic images coordinates in the world
    ...
    for i = 1:size(pano_poses,1)
       ...
       pano_xyz = R + pano_C(:,i);
       for j = 1:3
            plot3([pano_C(1,i) pano_xyz(1,j)],[pano_C(2,i) pano_xyz(2,j)],[pano_C(3,j) pano_xyz(3,j)],[rgb(j) '-'],'LineWidth',3);
       end
       text(pano_C(1,i),pano_C(2,i),pano_xyz(3,j)+0.5,pano_images{i},'Color',[1 0 0],'FontWeight','bold');
    end
end
